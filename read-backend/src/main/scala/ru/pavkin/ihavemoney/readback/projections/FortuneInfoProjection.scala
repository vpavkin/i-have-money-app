...
import ru.pavkin.ihavemoney.domain.fortune.FortuneProtocol.{EditorAdded, FortuneCreated, FortuneEvent, FortuneInitializationFinished}
  ...
  def handleEvent: HandleEvent = {
    ...
    case evt: FortuneEvent ⇒ evt match {
      case e: FortuneCreated ⇒
        repo.insert(e.aggregateId, FortuneInfo(e.aggregateId.value, e.owner.value, Set.empty, initializationMode = true))
      case e: EditorAdded ⇒
