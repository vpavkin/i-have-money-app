...
object AjaxExtensions {
  ...
  type RequestResult[T] = Xor[RequestError, T]

  private def toJson[T: Encoder](v: T): String = v.asJson.noSpaces

  private def defaultHandler(xhr: dom.XMLHttpRequest): Xor[RequestError, String] =
    if (xhr.status / 100 == 2) {
      xhr.responseText.right
    } else
      HTTPError(xhr.status, decode[FailedRequest](xhr.responseText).toOption, xhr.responseText).left

  private def ajaxExceptionHandler(handler: dom.XMLHttpRequest ⇒ RequestResult[String]): PartialFunction[Throwable, RequestResult[String]] = {
  ...
  def get(url: String,
          ...
          withCredentials: Boolean = false)
         (implicit ec: ExecutionContext): Future[RequestResult[String]] =
    Ajax.get(url, null, timeout, headers, withCredentials)
      .map(defaultHandler)
      .recover(ajaxExceptionHandler(defaultHandler))
